...
class Day03 {
    ...
    private fun findHighest(batteryarray: String): Pair<Int, Long> {
        return batteryarray.mapIndexed { index, battery -> Pair(index, battery.digitToInt().toLong()) }
            .maxBy { it.second }
    }
    ...
    private fun findSuperBattery(size: Int, batteryarray: String): Long {
        return if (size == 1) {
            ...
            return (highest.second.toString() + findSuperBattery(
                size - 1,
                batteryarray.drop(highest.first + 1)
            )).toLong()
        }
